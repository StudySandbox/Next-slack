  ...
  handler: async (ctx, args) => {
    ...
    const currentMember = await ctx.db
      ...
      .withIndex("by_workspace_id_user_id", (query) => {
        return query.eq("workspaceId", member.workspaceId).eq("userId", userId);
      });
...
export const current = query({
  ...
  handler: async (ctx, args) => {
    ...
    const member = await ctx.db
      .query("members")
      .withIndex("by_workspace_id_user_id", (query) =>
        query.eq("workspaceId", args.workspaceId).eq("userId", userId),
      )
      .unique();
...
export const remove = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const [messages, reactions, conversations] = await Promise.all([
      ctx.db
        .query("messages")
        .withIndex("by_member_id", (query) => query.eq("memberId", member._id))
        .collect(),
      ctx.db
        .query("reactions")
        .withIndex("by_member_id", (query) => query.eq("memberId", member._id))
      ...
      ctx.db
        ...
        .filter((query) =>
          query.or(
            query.eq(query.field("memberOneId"), member._id),
            query.eq(query.field("memberTwoId"), member._id),
          ),
        )
        .collect(),
    ]);
